...
The sidebar fields let you point the app at a different `nba_stats.db` path or a `predictions.csv` generated by `predict_top_3pm.py`. Deployments (e.g., Streamlit Community Cloud) only need the repo plus a copy of the SQLite database hosted alongside the app instance.

**Streamlit Cloud tip:** the repository does not contain `nba_stats.db` (it is larger than GitHub’s 100 MB limit). When the deployed app starts, upload your locally generated `nba_stats.db` through the sidebar file uploader, or provide a mounted path/URL so the dashboard can read the tables.

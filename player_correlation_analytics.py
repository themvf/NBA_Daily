...
def calculate_teammate_correlations(
    ...
    """
    Calculate how a player performs with each teammate.
    ...
    Returns:
        List of TeammateCorrelation objects, sorted by correlation_score (descending)
    """
    # Get all games the player participated in
    player_games_query = """
    SELECT game_id, points, fg3m, usg_pct, minutes, team_id
    ...
    team_id = player_games['team_id'].iloc[-1]

    # Get all teammates (players who played for the same team)
    teammates_query = """
    ...
    for _, teammate_row in teammates.iterrows():
        ...
        # Get games where this teammate played
        ...
        WHERE player_id = ?
          ...
          AND season_type = ?
        """
        teammate_games = pd.read_sql_query(
            teammate_games_query,
            conn,
            params=[teammate_id, season, season_type]
        )

        teammate_game_ids = set(teammate_games['game_id'].values)
        player_game_ids = set(player_games['game_id'].values)

        # Games where both played
        games_together_ids = player_game_ids & teammate_game_ids
        # Games where player played but teammate didn't
        games_apart_ids = player_game_ids - teammate_game_ids

        if len(games_together_ids) < min_games_together or len(games_apart_ids) < 1:
            continue

        # Calculate stats for games together
        together_games = player_games[player_games['game_id'].isin(games_together_ids)]
        avg_pts_together = together_games['points'].mean()
        avg_fg3m_together = together_games['fg3m'].mean()
        avg_usg_together = together_games['usg_pct'].mean()
        avg_minutes_together = together_games['minutes_decimal'].mean()

        # Calculate stats for games apart
        apart_games = player_games[player_games['game_id'].isin(games_apart_ids)]
        avg_pts_apart = apart_games['points'].mean()
        avg_fg3m_apart = apart_games['fg3m'].mean()
        avg_usg_apart = apart_games['usg_pct'].mean()
        avg_minutes_apart = apart_games['minutes_decimal'].mean()
        ...
        # Weighted by points delta (primary), 3PM delta (secondary), minutes stability
        correlation_score = _calculate_correlation_score(
            pts_delta, fg3m_delta, minutes_delta, len(games_together_ids)
        )

        correlations.append(TeammateCorrelation(
            teammate_id=teammate_id,
            teammate_name=teammate_name,
            games_together=len(games_together_ids),
            games_apart=len(games_apart_ids),
